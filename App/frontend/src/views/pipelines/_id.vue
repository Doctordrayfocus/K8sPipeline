  ...
  <div class="py-6 grid grid-cols-12 px-8 gap-4">
    <div class="md:col-span-8 col-span-1 flex flex-col space-y-5 items-center">
      ...
      <div class="w-full grid grid-cols-9 gap-5">
        <bp-build v-for="(build, index) in builds" :key="index" :build="build" @click="Logic.Common.GoToRoute('/builds/' + build.uuid)"> </bp-build>
      </div>
    </div>
    ...
    <div class="col-span-4 flex flex-col space-y-3 pb-5 pl-4">
      ...
      <div class="w-full flex flex-col bg-gray-100 rounded-md space-y-2" v-for="(setting, index) in buildSettings" :key="index">
        <div
          @click="setting.opened ? (setting.opened = false) : (setting.opened = true)"
        ...
        >
          ...
          <bp-icon :name="setting.opened ? 'angle-small-up' : 'angle-small-down'" custom-class="h-[23px]" />
        </div>
        ...
        <template v-if="setting.opened">
          <div class="w-full flex flex-col space-y-3 px-4 pb-4">
            <div class="w-full flex flex-row pt-2">
              <bp-select
                ...
                :default-values="setting.configToUse.length == 0 ? defaultConfigToUse : setting.configToUse"
            ...
            <template v-for="(value, key) in setting.serviceConfigs" :key="key">
              <div class="w-full flex flex-col" v-if="setting.variableToShow.includes(key.toString())">
                <bp-text-field :placeholder="`Enter value`" v-model="setting.serviceConfigs[`${key}`]" :has-title="true">
          ...
          <div class="w-full flex flex-col px-4 pb-6">
            <bp-button padding="py-3 px-5" @click="updatePipelineSetting(setting)"> Save </bp-button>
          </div>
        </template>
      </div>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const configTemplateToUse = JSON.parse(PipelineData?.lang_config_folders || '');
    ...
    const defaultConfigToUse = ref<any>(['/namespaces', `/${PipelineData?.lang}-default`]);

    let settingsPosition = 0;
    const handleSelectedConfigOptions = (selectData: { selectedItems: string[]; extraData: any }) => {
      buildSettings.value[selectData.extraData].variableToShow.length = 0;

      buildSettings.value[selectData.extraData].selectedConfigToUse = selectData.selectedItems;

      selectData.selectedItems.forEach(configKey => {
        configTemplateToUse[configKey].forEach((variable: string) => {
          if (!buildSettings.value[selectData.extraData].variableToShow.includes(variable) && !variableToIgnore.includes(variable)) {
            buildSettings.value[selectData.extraData].variableToShow.push(variable);
          }
        });

        defaultConfigToUse.value = ['/namespaces', `/${PipelineData?.lang}-default`];
      });
    };
    ...
    const updatePipelineSetting = (buildSetting: any) => {
      ...
      pipelineSettingsForm.configToUse = JSON.stringify(buildSetting.selectedConfigToUse);
      pipelineSettingsForm.serviceConfig = JSON.stringify(buildSetting.serviceConfigs);
    ...
    PipelineData?.builds?.forEach(eachBuild => {
      builds.value.push({
        title: eachBuild?.title || '',
        branch: eachBuild?.branch || '',
        color: Logic.Pipeline.pipelineStatusLabel(eachBuild?.status || '').color,
        duration: Logic.Common.timeFromNow(parseInt(eachBuild?.started_at || '0')),
        percentage: '40',
        status: eachBuild?.status || '',
        uuid: eachBuild?.uuid,
      });
    });
