import { Pipeline as PipelineData, CreatePipelineDto, SettingsUpdateData } from './../gql/graphql';
...
export default class Pipeline extends Common {
    ...
    public pipelineStatusLabel = (pipeline: PipelineData) => {
        let status = 'Active';
        let color = 'gray'
    ...
    public CreateServicePipeline = (formIsValid: boolean) => {
        if(formIsValid) {
           return $api.pipeline.createServicePipeline(this.CreatePipelineForm).then((response) => [
                this.EachPipeline = response.data?.createPipelineTemplate
            ])
        }
    }
